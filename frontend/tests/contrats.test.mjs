...
// Entreprise ne charge plus ses huit onglets d'un bloc : ouvrir la vue
// declenchait sept appels alors qu'un seul panneau est visible. Chaque onglet
// a maintenant son chargeur, joue a sa premiere ouverture. L'intention tenue
// ici reste la meme : atteindre l'onglet Contrats charge bien les contrats.
assert.match(
  appSource,
  /contrats: \(\) => loadContrats\(\)/,
  "l'onglet Contrats doit avoir son chargeur",
);
assert.match(
  appSource,
  /entreprise: \(\) => chargerOngletEntreprise\(ongletEntrepriseActif\(\)\)/,
  "ouvrir la vue doit charger l'onglet actif",
);
assert.match(
  appSource,
  /afficherOngletEntreprise\(btn\.dataset\.tab\);\s*\n\s*chargerOngletEntreprise\(btn\.dataset\.tab\);/,
  "changer d'onglet doit charger le sien",
);
// Un echec ne doit pas condamner l'onglet a rester vide pour toujours.
assert.match(
  appSource,
  /ongletsEntrepriseCharges\.delete\(cle\);/,
  "un chargement en echec doit pouvoir etre retente",
);
